...
object UserCreationExercises {
  ...
  def readSubscribeToMailingList(): Boolean = {
    ...
    val reply = StdIn.readLine()
    parseYesNo(reply)
  }

  def parseYesNo(reply: String) = {
    reply match {
      case "Y" => true
      case "N" => false
      case other => throw new IllegalArgumentException(s"""Must be "Y" or "N" but received $other""")
    }
  }
  ...
  def readUser(console: Console, clock: Clock): User = {
    ...
    val dob = readDateOfBirthRetry(console, 3)
    val subscribed = readSubscribeToMailingListRetry(console, 3)
    val user = User(name, dob, subscribed, clock.now())
